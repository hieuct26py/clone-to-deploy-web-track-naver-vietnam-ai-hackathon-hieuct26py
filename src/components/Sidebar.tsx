...
import logo from '../assets/todue-logo.svg'
import { HomeIcon, ListIcon, SunIcon, ArrowUpRightIcon, CheckCircleIcon, CloudIcon, CloudRainIcon, SnowIcon, StormIcon, PlusIcon, PencilIcon } from '../assets/icons'
import { useConfirm } from '../ui/Confirm'
import type { View } from '../types'

export function Sidebar({ current, open, onClose, onNavigate }: { current: View; open: boolean; onClose: () => void; onNavigate: (v: View) => void }) {
  ...
  useEffect(() => {
    const measure = () => {
      ...
      else if (current.type === 'upcoming') el = upcomingBtn.current
  ...
  return (
    <aside className={`sidebar ${open ? 'open' : 'collapsed'}`}>
      ...
      <div className="sidebar-section nav-stack" ref={navRef}>
        ...
        <button ref={upcomingBtn} className={`nav-item ${isActiveRoot('upcoming') ? 'active' : ''}`} onClick={() => go({ type: 'upcoming' })}><ArrowUpRightIcon className="nav-ico" /><span>Upcoming</span></button>
