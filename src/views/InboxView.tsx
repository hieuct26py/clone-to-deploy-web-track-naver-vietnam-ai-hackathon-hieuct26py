...
import { dateKey, formatDateOnly, isFuture, isPast, isToday } from '../utils/date'
...
export function InboxView({ title = 'All Tasks', search }: { title?: string; search?: string }) {
  ...
  const { overdue, today, upcomingMap, nodate, upcomingKeys } = useMemo(() => {
    let arr = tasks.filter((t) => !t.completed)
    if (search) arr = arr.filter((t) => t.title.toLowerCase().includes(search.toLowerCase()))

    const overdue = arr.filter((t) => t.dueDate && isPast(t.dueDate) && !isToday(t.dueDate)).sort(sort === 'time' ? sortByTime : sortByPriority)
    const today = arr.filter((t) => t.dueDate && isToday(t.dueDate)).sort(sort === 'time' ? sortByTime : sortByPriority)
    const nodate = arr.filter((t) => !t.dueDate).sort(sort === 'time' ? sortByTime : sortByPriority)
    const upcoming = arr.filter((t) => t.dueDate && isFuture(t.dueDate))
    ...
    return { overdue, today, upcomingMap, nodate, upcomingKeys }
  }, [tasks, search, sort])
  ...
  return (
    <section>
      ...
      {overdue.length > 0 && (
        <div className="group">
          <div className="group-title">Overdue</div>
        ...
        <div className="group">
          ...
          {upcomingKeys.map((k) => (
            <div key={k} className="group" style={{ marginTop: 8 }}>
              ...
              <TaskList tasks={upcomingMap.get(k)!} />
            </div>
          ))}
        </div>
      )}
      ...
      {overdue.length + today.length + upcomingKeys.length + nodate.length === 0 && (
